...
def cars():
    # Keyin shu ro'yxatni sizning haqiqiy mashinalaringiz bilan almashtiramiz.
...
async def choose_given(update: Update, context: ContextTypes.DEFAULT_TYPE):
    buttons = [[InlineKeyboardButton(c, callback_data=f"given_car:{c}")]
               for c in cars()]
...
async def choose_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    buttons = [[InlineKeyboardButton(c, callback_data=f"received_car:{c}")]
               for c in cars()]
    context.user_data.clear()
...
async def date_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [[InlineKeyboardButton(c, callback_data=f"{mode}_car:{c}")]
               for c in cars()]
    await update.message.reply_text(
        "🚛 Mashinani tanlang:",
...
async def car_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        f"🚛 Mashina: {car}\n💰 Summani kiriting (masalan: 180000 yoki 180.50):"
    )
    return 3
...
async def amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "given":
        ...
        # Eng oxirgi to'lanmagan berilgan yozuvni shu mashina bo'yicha topamiz.
        row = con.execute(
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Farqi: {fmt(row[2])}\n"
        f"Berilmagan/olinmagan yozuvlar: {pending}"
    )
