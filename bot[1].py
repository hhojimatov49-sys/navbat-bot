...
def report_text():
    ...
    for truck in TRACTORS:
        ...
        lines.append(f"   ➕ Berildi : {fm(given)}")
        lines.append(f"   💰 Olindi  : {fm(received)}")
        lines.append(f"   📊 Farq    : {fm(diff)}")
        lines.append("")

    total_diff = total_given - total_received

    lines += [
        "━━━━━━━━━━━━━━━━━━━━",
        "📌 JAMI NATIJA",
        f"➕ Berilgan : {fm(total_given)}",
        f"💰 Olingan  : {fm(total_received)}",
        f"📊 Farq     : {fm(total_diff)}",
    ]
